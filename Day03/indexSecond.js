const genrerateWireCoordinates = str => {
    let current = [0,0];
    const result = [];
    str.split(',').forEach(segment => {
        ...
        for (let i = 0; i < distance; i++) {
            if (direction === 'U') { current[0] += 1; }
            else if (direction === 'R') { current[1] += 1; }
            else if (direction === 'D') { current[0] -= 1; }
            else if (direction === 'L') { current[1] -= 1; }
            result.push([current[0], current[1]]);
        }
    });
    return result;
}
const generateBands = arr => {
    return arr.reduce((accum, point) => {
      const max = Math.max(Math.abs(point[0]), Math.abs(point[1]));
      if (!accum[max]) { accum[max] = []; }
      accum[max].push(point)
      return accum;
    }, {})
}
const calculateManhattanDistance = (p, q) => Math.abs(p[0] - q[0]) + Math.abs(p[1] - q[1]);
const findCloestDistance = arr => {
    return arr.reduce((accum, intersection) => {
        const manDist = calculateManhattanDistance([0,0], intersection);
        if (accum > manDist ) { 
            console.log('NEW CLOSEST POINT FOUND! -- ', manDist);
            accum = manDist; 
        }
        return accum;
    }, Infinity);
...
const generateIntersections = (one, two) => {
    ...
    let uniqueWireTwo = new Set(wireTwo);
    uniqueWireOne = Array.from(uniqueWireOne).sort();
    uniqueWireTwo = Array.from(uniqueWireTwo).sort();
    const oneBanded = generateBands(uniqueWireOne);
    const twoBanded = generateBands(uniqueWireTwo);
    const keys = Object.keys(oneBanded);
    const intersections = [];
